...
public class BingoCardTests {
	...
	public void eachNumberOnCardIsUnique() {
		for (int i = 0; i < 10; i++) {
			...
			assertTrue(String.format("Card numbers are not unique: %s", String.join(" ", card)),
					set.size() == card.length);
		}
	}
	...
	private void checkCategory(String[] card, String column, int start, int end) {
		for (int i = start - 1; i < end; i++) {
			assertTrue(String.format("Number should start with '%s', found: '%s')", column, card[i]),
					card[i].startsWith(column));
		}
	}
	...
	private void checkColumn(String[] card, String column, int start, int end, int beginRange, int endRange) {
		for (int i = start - 1; i < end; i++) {
			...
			assertTrue(String.format("Number should be in range from %d to %d, found: '%s')", beginRange, endRange,
					card[i]), n >= beginRange && n <= endRange);
		}
	}
	...
	public void numbersWithinColumnAreInRandomOrder() {
		...
		int count = checkColumnOnRandomness(card, "B", 1, 5) + checkColumnOnRandomness(card, "I", 6, 10)
				+ checkColumnOnRandomness(card, "N", 11, 14) + checkColumnOnRandomness(card, "G", 15, 19)
